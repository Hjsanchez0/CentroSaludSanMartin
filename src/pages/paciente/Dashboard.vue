  ...
  <div>
    ...
    <div class="container">
      <div class="content">
        <h3>INICIO</h3>
        <div class="user-info">
          <label>SIS: {{ userData.sis }}</label>
          <label>Historial clínico: {{ userData.historialClinico }}</label>
        </div>
      </div>
      <div class="profile-container">
        <div class="profile">
          ...
          <hr class="linea">
          <button class="cita" @click="viewAppointments"><img src="../../assets/cita.png" alt="Appointment Icon"
              class="button-icon">Mis Citas</button>
          <hr class="linea">
        ...
        <div class="button-container">
          <div class="button-box">
            <button class="create-appointment" @click="showCreateAppointment">Crear Cita</button>
          </div>
          <div v-if="showAppointmentsInfo" class="button-box">
            <ViewAppointments :userData="userData" />
          </div>
        </div>
    ...
    <CreateAppointment :visible="isCreateAppointmentVisible" @close="isCreateAppointmentVisible = false" />

    <div v-if="showSisInactiveModal" class="modal-overlay">
      <div class="modal">
        <img src="../../assets/exclamacion.png" alt="SIS Inactivo" class="sis-image">
        <p>No cuenta con su SIS activo, por lo que la cita tendrá un valor de S/. 20 soles.</p>
        <p>Se recomienda esperar 1 día hábil si el estado del SIS dice "Procesando..."</p>
        <p>Hasta que el estado del SIS pase a "Activo" para que no tenga ningún costo.</p>
        <p>Caso contrario, se le pide acercarce al Centro de Salud San Martin de Porres.</p>
        <button @click="cancelSisInactiveModal">Cancelar</button>
        <button @click="acceptSisInactiveModal">Aceptar</button>
      </div>
    </div>
  </div>
</template>
...
import CreateAppointment from '../../components/paciente/CreateAppointment.vue';
import ViewAppointments from '../../components/paciente/ViewAppointments.vue';
...
export default {
  ...
  data() {
    return {
      ...
      isCreateAppointmentVisible: false,
      showSisInactiveModal: false,
      showAppointmentsInfo: false
    };
  },
  created() {
    this.getUserData();
  ...
  methods: {
    ...
    showCreateAppointment() {
      if (
        this.userData.sis === "Inactivo" ||
        this.userData.sis === "Procesando..." ||
        this.userData.sis === "No tiene"
      ) {
        this.showSisInactiveModal = true;
      } else {
        this.isCreateAppointmentVisible = true;
      }
    },

    acceptSisInactiveModal() {
      this.showSisInactiveModal = false;
      this.isCreateAppointmentVisible = true;
    },

    cancelSisInactiveModal() {
      this.showSisInactiveModal = false;
    },
    ...
    viewAppointments() {
      this.showAppointmentsInfo = true;
    }
  }
};
</script>
